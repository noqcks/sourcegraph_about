...
const mapSlugDataCache = async (baseDirectory: string): Promise<SlugCacheObject> => {
    ...
    const returnDirectorySlugs = async (directory: string): Promise<SlugObject> => {
        const directoryFiles = await globby('**/*.md', { cwd: path.join(baseDirectory, directory), objectMode: true })
        const slugs = {
            contentDirectory: directory,
            recordSlugs: Object.fromEntries(
                await Promise.all(
                    directoryFiles.map(async (file) => [
                        await loadSlug(path.join(directory, file.path)),
                        {
                            slugPath: await loadSlug(path.join(directory, file.path)),
                            publishDate: await loadDate(path.join(directory, file.path)),
                        },
                    ])
                )
            ) as SlugRecord,
